...
To test whether the mean number of children ever born differs across education levels, we assume the following fixed-effects one-way ANOVA model:
...
where:
\begin{itemize}
    \item $Y_{ij}$ = the $j^{th}$ observation of Children Ever Born in the $i^{th}$ education group,
    \item $\mu$ = overall population mean,
    \item $\alpha_i$ = effect of the $i^{th}$ education level (group effect),
    \item $\varepsilon_{ij}$ = random error term with $\varepsilon_{ij} \sim N(0, \sigma^2)$.
\end{itemize}

The hypotheses for ANOVA are:
\begin{align*}
    H_0 &: \alpha_1 = \alpha_2 = \cdots = \alpha_k = 0 
    && \text{(all education levels have equal mean fertility)} \\
    H_1 &: \text{At least one } \alpha_i \neq 0
    && \text{(mean fertility differs between groups)}.
\end{align*}
...
and the F-statistic is computed as:
\begin{equation}
...
where:
\begin{itemize}
    \item $k$ = number of education groups,
    \item $N$ = total number of observations.
\end{itemize}

If the computed $p$-value from the F-statistic is less than the significance level $\alpha = 0.05$, we reject the null hypothesis and conclude that education level has a significant effect on fertility.
...
\end{figure}
\begin{remark}
    The codes (like 11, 12, $\ldots$ , 97) are categorical identifiers for each specific toilet facility type (for example: flush to sewer, flush to septic tank, pit latrine with slab, pit latrine without slab, no facility/open defecation, etc.). They allow the survey to capture a wide variety of facility types, including both improved and unimproved categories, and variations such as whether the facility is shared, whether it is located within the house/plot or outside, etc.
\end{remark}
